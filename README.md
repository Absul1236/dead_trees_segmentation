...
├── copy_config.yaml         # template configuration file (read-only reference)
├── README.md
├── requirements.txt
├── .gitignore
├── data/
...
Optional demonstration samples can be placed in `examples/` subfolders.
...
The file `copy_config.yaml` is a **template configuration file**.
...
The separation between `copy_config.yaml` and `config.yaml` prevents accidental modification
